...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "Contract not called by owner");
        _;
    }
    //  4) fill out the transferOwnership function
    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event ownershipTransferred(address indexed originalOwner, address indexed newOwner);
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(_paused == false, "Paused must be false");
        _;
    }

    modifier paused() {
        require(_paused == true, "Paused must be true");
        _;
    }
...
contract ERC721 is Pausable, ERC165 {
    ...
    function balanceOf(address owner) public view returns (uint) {
         require(owner != address(0), "ERC721: balance for zero address");
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != owner, "Given address must not be owner of tokenId");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == owner || isApprovedForAll(owner, msg.sender), "Owner of contract is msg.sender or isApprovedforAll()");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        _ownedTokensCount[_tokenOwner[tokenId]].increment();
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "From address must be owner");
        // TODO: require token is being transfered to valid address
        require(to != address(0), "Must send token to valid address");
        // TODO: clear approval
...
contract RealEstateERC721Token is ERC721Metadata("RealEstateCoin", "REC", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
